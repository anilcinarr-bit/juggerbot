...
from app.platforms.trendyol import HOME_URL, COUPON_PAGE_URL, COUPON_INPUT_SELECTOR, APPLY_BUTTON_SELECTOR, RESULT_SELECTOR
...
class TrendyolAdapter(BaseAdapter):
    ...
    async def execute(self, coupon: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Navigate to Trendyol website
            logger.info("Opening Trendyol...")
            ...
            if success_message:
                ...
                if "kullanılmış" in error_text.lower() or "already used" in error_text.lower():
                    return {
                        "success": False,
                ...
                elif "geçersiz" in error_text.lower() or "invalid" in error_text.lower():
                    return {
                        "success": False,
